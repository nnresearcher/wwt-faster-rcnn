...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 12 09:31:10 2018
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    ...
    # y_rpn_overlap: num_anchors * pic
    # y_is_box_valid: num_anchors * pic, which anchor is valid
    # y_rpn_regr: 4 * num_anchors * pic, every anchor's boxes dx, dy, dw, dh
    y_rpn_overlap = np.zeros((output_height, output_width, num_anchors))
    y_is_box_valid = np.zeros((output_height, output_width, num_anchors))
    y_rpn_regr = np.zeros((output_height, output_width, num_anchors * 4))
    # in pic has how many bboxes
    num_bboxes = len(img_data['bboxes'])
    # num_bboxes: how many bboxes in pic
    # num_anchors_for_bbox: num_bboxes
    # best_anchor_for_bbox: each bboxes has 4 info x1, x2, y1, y2
    # best_iou_for_bbox: best iou for every bboxes
    # best_x_for_bbox:
    # best_dx_for_bbox: best modify info
    num_anchors_for_bbox = np.zeros(num_bboxes).astype(int) # if 'pos', val=1
    best_anchor_for_bbox = -1*np.ones((num_bboxes, 4)).astype(int) #[jy, ix, anchor_ratio_idx, anchor_ratios]
        ...
        for anchor_ratio_idx in range(len(anchor_ratios)):
            ...
            for ix in range(output_width):
                ...
                for jy in range(output_height):
                    ...
                    best_iou_for_loc = 0.0# best iou for (x,y) not for GT box
                    ...
                    for bbox_num in range(num_bboxes):
                        ...
                        if img_data['bboxes'][bbox_num]['class'] != 'bg':
                            if curr_iou > best_iou_for_bbox[bbox_num]:
                                best_anchor_for_bbox[bbox_num] = [jy, ix, anchor_ratio_idx, anchor_ratios]
    ...
    for idx in range(num_bboxes):
        if num_anchors_for_bbox[idx] == 0:        # nothing box is 'pos'
            if best_anchor_for_bbox[idx, 0] ==-1: #class=='bg', value=-1
                continue
    ...
    while True:
        ...
        for img_data in all_img_data:
            try:
                ...
                (resized_width, resized_height) = get_new_img_size(width, height, cfg.im_size)
