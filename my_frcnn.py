...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 11 19:22:15 2018
...
# our packages
from keras_frcnn_help import data_generators
from keras_frcnn_help import losses as losses_fn
from keras_frcnn_help.Config import config
from keras_frcnn_help.get_data import get_data
from keras_frcnn_help import resnet as nn
import keras_frcnn_help.roi_helpers as roi_helpers
...
def train_frcnn():
    cfg=config()#get some config info
    
    ###############################
    ### set some cfg info again ###
    ###############################
    
    # how to augment pic data 
    cfg.use_horizontal_flips = True
    cfg.use_vertical_flips = True
    cfg.rot_90 = True

    # simple_label_file contain image path, object position and name
    ...
    # all_images: dict in list, every dict obtain one image info   ##
    #             how many object and how many class               ##    
    # class_count: dict, key is class's name, value is class's num ##
    # class_mapping: every class's lag                             ##  
    #################################################################
    
    
    all_images, classes_count, class_mapping = get_data.get_data(cfg.simple_label_file)
    ...
    # print some info
    print('training image per class:')
    pprint.pprint(classes_count)
    print("Number class (include 'bg') = {}".format(len(class_mapping)))
    
    # inv map, key is lag, value is class_name
    #inv_map = {v: k for k, v in class_mapping.items()}
    
    # shuffle the order of all_images
    ...
    #base network
    shared_layers = nn.nn_base(image_input, trainable=True)
    ...
    model_rpn.compile(optimizer=optimizer,
                      loss=[losses_fn.class_loss_cls(num_anchors), losses_fn.rpn_loss_regr[num_anchors]])
    model_classifier.compile(optimizer=optimizer_classifier,
                             loss=[losses_fn.class_loss_cls, losses_fn.class_loss_regr(len(classes_count) - 1)],
                             metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
    
    model_all.compile(optimizer='sgd', lose='mae')
    epoch_length = 1000
    num_epochs = int(cfg.num_epochs)
    iter_num = 0
    
    losses = np.zeros((epoch_length, 5))
    rpn_accuracy_for_monitor = []
    ...
    for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                if len(rpn_accuracy_for_monitor) == epoch_length and cfg.verbose:
                    mean_overlapping_bboxes = float(sum(rpn_accuracy_for_monitor))/len(rpn_accuracy_for_monitor)
                    rpn_accuracy_for_monitor = []
                print(
                ...
                if X2 is None:
                    rpn_accuracy_for_monitor.append(0)
                ...
                rpn_accuracy_for_monitor.append((len(pos_samples)))
